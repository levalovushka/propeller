...
## Что изменилось 2026-07-28 (проверить в первую очередь)
- [ ] Звонок в **Контур.Толк** → запись стартует сама (идентификаторы не проверены на живой установке — если не сработало, снять `./tools/detect-meeting-signals.sh толк talk kontur` во время созвона)
- [ ] Zoom по-прежнему детектится; холостые панели («Настройки», «Чат») запись **не** запускают
