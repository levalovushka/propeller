...
public struct NoteBar: View {
    ...
    private var field: some View {
        sizer
            // Рост — только у мерки, и это не мелочь. Та же анимация, надетая на
            // всю плашку, обнимала и редактор: на каждом переносе SwiftUI снимал
            // с него кадр и переводил старый в новый, отчего текст моргал прямо
            // во время набора. Мерка пустая и невидимая — переводить в ней
            // нечего, а высоту она задаёт ту же самую.
