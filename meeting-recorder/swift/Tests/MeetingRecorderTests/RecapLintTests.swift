...
final class RecapLintTests: XCTestCase {
    ...
    func testПризрачныйИсполнительУходитАЗадачаОстаётся() {
        let recap = "## Задачи\n- **Система аналитики** — доработать плашки.\n"
    ...
    /// Разбор 2026-08-13: «Команда VK Музыка» — не выдумка, а сторона встречи,
    /// те, кому показывали работу. Пока это была находка для редактора, ошибка
    /// стоила пропущенной правки; вырезка из той же ошибки делает потерю правды,
    /// поэтому коллективный исполнитель остаётся в тексте.
    func testКоллективныйИсполнительОстаётся() {
        for line in [
            "- **Команда VK Музыка** — согласовать форму отображения кластеров.",
            "- **Команда дизайна** — доработать плашки.",
            "- **Участники** — прислать правки.",
        ] {
            XCTAssertEqual(grounded(line, transcript: ""), line)
        }
    }

    /// Но находкой для редактора он быть не перестал: тот вправе переписать
    /// «Команду» на имя, если имя в конспекте есть.
    func testКоллективныйИсполнительОстаётсяНаходкой() {
        XCTAssertTrue(kinds("- **Команда** — доработать плашки.").contains(.ghostOwner))
    }
    ...
    func testИмяИзСкобкиСтановитсяОтветственным() {
        XCTAssertEqual(
            grounded("- **Система аналитики (Левон)** — посчитать запуски.", transcript: ""),
            "- **Левон** — посчитать запуски."
        )
    }

    /// Дословно из прогонов решётки (`tools/recap-lab`, 2026-08-12 и живой
    /// прогон 2026-08-13): метка дорожки и несуществующая система уходят,
    /// сторона встречи остаётся.
    func testРеальныеСтрокиИзПрогонов() {
        for line in [
            "- **Speaker S1** — доработать макет главной страницы.",
            "- **Система аналитики** — проанализировать данные о запусках.",
        ] {
            let cut = grounded(line, transcript: "")
            XCTAssertFalse(cut.contains("**"), "исполнитель остался: \(cut)")
            XCTAssertTrue(cut.hasPrefix("- "), "пункт потерян: \(cut)")
        }
        let party = "- **Команда PG x VK Музыка** — реализовать облегчённый рич-текст."
        XCTAssertEqual(grounded(party, transcript: ""), party)
    }
