...
class Preferences {
    ...
    /// Показывать ли чёлку во время записи.
    ///
    /// Единственная поверхность, которую приложение рисует поверх чужих окон, —
    /// значит и единственная, у которой обязан быть выключатель. Выключенная,
    /// она уносит с собой и ⌃⌥N: заметка без видимого поля — это клавиатура,
    /// проваливающаяся в никуда. Запись, её индикация в меню-баре и заметки в
    /// окне при этом не меняются ничем.
    var notchIndicator: Bool {
        get {
            defaults.object(forKey: "notchIndicator") as? Bool ?? true
        }
        set { defaults.set(newValue, forKey: "notchIndicator") }
    }
    ...
    /// Zoom-имя владельца, выученное журналом окна.
    ///
    /// Находится корреляцией с микрофонными репликами на встречах, где Zoom
    /// пишет метку говорящего (`CallWindowJournal.LiveSpeaker.ownerZoomName`),
    /// и переиспользуется там, где метки нет: на встрече 1×1 плиток две,
    /// метки ноль (замер 2026-08-20, `20260820_213214`), и якорь «какая
    /// плитка моя» — единственное, что превращает вторую плитку в имя
    /// собеседника. Никогда не спрашивается у человека и никогда не попадает
    /// в ленту — только исключает и опознаёт.
    var ownerZoomName: String? {
        get { defaults.string(forKey: "ownerZoomName") }
        set { defaults.set(newValue, forKey: "ownerZoomName") }
    }
