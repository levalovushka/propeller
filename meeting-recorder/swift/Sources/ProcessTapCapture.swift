...
final class ProcessTapCapture {
    ...
    private func buildAndStart(isRestart: Bool) throws {
        ...
        // Строится до таймера дренажа: первый `AVAudioConverter` 48k→16k занимает
        // десятки миллисекунд, а таймер бьёт через 50 мс — раньше здесь, дренаж
        // не застанет `resample` с конвертером ещё нулевым.
        prepareConverters()
        if !isRestart {
