...
public enum MCPCellState: String, CaseIterable, Equatable, Sendable {
    ...
    ///
    /// Подписи нет у двух состояний, и оба раза потому, что её работу уже
    /// делает то, что стоит справа. У отказа записи — кнопка: причина уехала в
    /// телеметрию, разбираемся мы, а «не получилось» слово в слово повторяло бы
    /// «Попробовать снова». У подключённого — галочка: «Подключён» под
    /// заголовком и галочка справа говорят одно и то же дважды, и из двух
    /// одинаковых утверждений лишним оказывается слово (решение владельца
    /// 2026-08-19, по снятым состояниям).
