...
/// # Что говорит одна ячейка в настройках
///
...
public enum ClaudeCellState: String, CaseIterable, Equatable, Sendable {
    ...
    /// Строка под заголовком «Claude» — единственное, что меняется.
    public var subtitle: String {
        switch self {
        case .notInstalled:  return "Нужен Claude Desktop — приложение с сайта Anthropic"
        case .offer:         return "Claude сможет читать ваши встречи"
        case .restartNeeded: return "Перезапустите Claude, чтобы он увидел встречи"
        case .connected:     return "Claude подключён. Спросите его о встречах"
