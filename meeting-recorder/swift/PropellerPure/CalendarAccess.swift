...
/// начинает придумывать модель — молча. Так и случилось у автора 2026-08-17:
/// сменилась подпись сборки, грант TCC перестал совпадать с новой личностью
/// приложения, и три дня встречи назывались «Запись 18.08.2026, 14:04», притом
/// что в System Settings галка стояла — она была про прежнюю подпись. Фича
/// отвалилась без единого свидетеля. Строка и есть свидетель.
///
...
public enum CalendarSettingsRow: Equatable, Sendable {
    ...
    case granted
    /// Приложение календарь ждёт, а система ещё не спрошена — так выглядит
    /// грант, слетевший от смены подписи сборки. Окно покажется, поэтому
    /// спрашиваем сами, а не отправляем человека в System Settings.
    case ask
    ...
    public var subtitle: String? {
        ...
        case .ask:     return "Доступ сбросило обновление — названия придумывает модель"
