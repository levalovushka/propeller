...
public struct CaptureCursor {
    ...
    /// Снять шкалу с якоря намеренно: запись поставили на паузу, и время, пока
    /// она стояла, не должно попасть в файл ни тишиной, ни сдвигом.
    ///
    /// Отличается от `reanchor()` ровно одним — не считается сбоем. «Сколько раз
    /// за встречу поехали часы» обязано остаться ответом про здоровье захвата, а
    /// не про то, сколько раз человек нажал паузу.
    public mutating func detachClock() {
        expectedSampleTime = nil
    }
