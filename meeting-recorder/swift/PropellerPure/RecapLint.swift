...
public enum RecapLint {
    ...
    /// Кого вырезаем — **только заведомо не людей**, и граница здесь проведена
    /// не из осторожности, а по разбору 2026-08-13.
    ///
    /// Пока `ghostOwner` был находкой для редактора, широкий список стоил
    /// пропущенной правки. Как только тот же список стал удалением
    /// (`grounded(_:transcript:)`), цена ложного срабатывания перевернулась:
    /// оно теперь стирает правду. И первый же живой прогон это показал —
    /// «**Команда VK Музыка** — согласовать форму» я счёл выдумкой, а это
    /// сторона встречи: те, кому показывали работу, и в шапке транскрипта их
    /// домены. Удалить такого исполнителя хуже, чем оставить.
    ///
    /// Поэтому вырезаются два вида, про которые спорить не о чем:
    /// **система** (не люди вовсе) и **метка диаризации** (`Speaker S1` — не имя,
    /// а номер дорожки; ею подписала задачу qwen2.5:7b). Коллективные —
    /// «Команда …», «Участники», «Все», «Разработка» — остаются в тексте и
    /// остаются находкой для редактора: он их перепишет или не перепишет, но
    /// потерять их он не может.
    private static let ghostOwnerHead =
        #"(?m)^(\s*-\s+)\*{0,2}(?:Система(?:\s+[^\-–—*\n(]{1,40})?|Спикер\s*S?\d+|Speaker\s*S?\d+)\*{0,2}\s*[—–-]\s*"#

    /// «**Система аналитики (Левон)** — посчитать» → «**Левон** — посчитать».
    /// Скобка — единственное, что модель про ответственного знала; снести
    /// заголовок целиком значило бы выбросить её вместе с ним.
    private static let ghostOwnerWithNames =
        #"(?m)^(\s*-\s+)\*{0,2}(?:Система|Спикер|Speaker)[^*\n(]{0,40}\(([^)\n]{1,60})\)\*{0,2}(\s*[—–-]\s*)"#
    ...
    /// которое исполняется через раз, — не правило, а пожелание.
    ///
    /// И ровно поэтому вырезать можно только то, о чём спорить не о чем:
    /// удаление не переживает ложного срабатывания так, как переживала находка.
    /// Границу держит `ghostOwnerHead` — см. разбор над ним.
    ///
    ...
    ///
    /// Совпадение по токенам, не по строке: «Марина» обязана пройти против
    /// «Marina Primer» не всегда (кириллица против латиницы — это и есть
    ...
    /// общий префикс от четырёх букв: падеж меняет хвост («Марине» / «Марина»),
    /// голова остаётся.
