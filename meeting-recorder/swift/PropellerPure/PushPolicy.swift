...
public enum PushPolicy {
    ...
    public enum Surface: Equatable, Sendable {
        ...
        /// Уведомления запрещены, а повод такой, что молчать нельзя:
        /// показываем окно, чтобы действие было достижимо.
        case window
    ...
    public static func surface(for kind: Kind, in ctx: Context) -> Surface {
        switch kind {
        case .recordingStarted:
            // Без разрешения на уведомления действие «Не записывать» негде
            // нажать — тогда окно, иначе встреча пишется молча и без отказа.
            guard ctx.authorized else { return .window }
        ...
        case .micDenied:
            // В окне это строка «Начать запись», и она никуда не денется.
