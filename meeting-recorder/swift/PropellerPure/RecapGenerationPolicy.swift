...
public enum RecapGenerationPolicy {
    ...
    /// Доля пунктов механической сборки, ниже которой свод считается съевшим
    /// содержание и документ уходит сборке (`RecapDigestGuard`).
    ///
    /// Половина, а не две трети как у `RecapLint.Shape.lostContentComparedTo`:
    /// там сравниваются два состояния одного документа, здесь — документ и
    /// свалка находок, из которой он написан, и она заведомо длиннее.
    ///
    /// Оба замеренных схлопывания A5.1 (4 пункта против 9 и 5 против 8) ловит
    /// порог токенов: они пришли на 351 и 465 при пороге 800. Эта доля —
    /// вторая сеть, для ответа, который длину набрал и содержание всё равно
    /// выбросил, и половина — самый мягкий порог, отделяющий такой ответ от
    /// редактуры, слившей близкие пункты.
    public static let digestMinBulletShare = 0.5
